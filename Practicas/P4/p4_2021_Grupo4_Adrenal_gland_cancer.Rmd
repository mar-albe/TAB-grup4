...
```
```
Respuesta: En nuestro caso debemos eliminar 189 muestras porque no tienen un stage determinado
```
As we can see, all patients have information because all the results are `FALSE` when we check if something "_is na?_". But if we are dealing with some `TRUE` results (i.e., there's no data for some patients), we can remove it as:
...
```
``` 
Respuesta: Hemos tenido que eliminar 189 de los 266, aun así seguimos teniendo 77
```
...
head(resDF)
ggplot(data = resDF, mapping = aes(x = log2FoldChange, y = -log(padj), colour = filter)) +  geom_point()+
theme_bw(base_size=8) + scale_color_manual (values = c("#000000","#E69F00","#56B4E9","#009E73", "#F0E442","#0072B2","#D55E00","#CC79A7")) + labs(title = "Adrenal gland cancer RNA-seq analysis", subtitle = "Volcano plot" , x = "log2(fold-change)", y = "-log10(adjusted p-value)")
...
annotatedplot <- plot +theme(legend.position = "none")+ geom_label_repel(data=resDF[abs(resDF$log2FoldChange) > log2(10) & resDF$padj < 0.0001 ,], aes(label = as.factor(gene)), alpha = 0.6, size = 3, force = 1.1)  
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
La glándula adrenal debe responder a estímulos y químicos para poder producir bajo demanda del organismo las hormonas esteroides y ademas de la producción de neurotransmisores como la adrenalina y noradrenalina.otros procesos que vemos que son altamente significativos son los procesos metabólicos de diferentes productos quimicos.
</div>
