...
```
Las variables "shape", "size" y "alpha" no son variables discretas sino continuas. El error que sale Error: attempt to use zero-length variable name.

```

```{r groups-ggplot2}

#Fem una gràfica i en comptes de diferenciar per colors ho fem per formes (triangles, quadrats i rodones.)
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, shape = Species)) +
geom_point()



#Fem una gràfica i en comptes de diferenciar per colors ho fem per mida
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, size = Species)) +
geom_point()



#Fem una gràfica i en comptes de diferenciar per colors ho fem per alfa (transparencia)
ggplot(data = iris, mapping = aes(x = Sepal.Width, y = Sepal.Length, alpha = Species)) +
geom_point()
#El gràfic alpha no el recomana per una variable discreta
```
...
```{r}
#mirem les 6 primeres files del dataset
head(mtcars)
#esbrinem quin tipus de dades es mtcars
class(mtcars)
```
...
```{r barplot-ggplot2}
#creem una grafica amb les dades de mtcars i representem a l'eix de les x la cilindrada en forma de geom_bar. Com que és una gràfica simple, només representem a l'eix X la cilindrada que com hem vist abans només hi ha 3 opcions (4,6,8) per això només sortiran 3 barres

ggplot(data = mtcars, mapping = aes(x = cyl_f)) +
  geom_bar()

#de fet, usem geom_bar perquè aquesta serveix per una variable discreta (la cilindrada o cyl)


#De fet, si volem cambiar el color de les barres també ho podem fer afegint-ho als aes després de les dades seguit d’una coma i colour filtrant per cyl_factor que és la cilindrada.

ggplot(data = mtcars, mapping = aes(x = cyl_f, colour = cyl_f)) + geom_bar()
...
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = am_f)) + geom_bar(position = "dodge")

ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = am_f)) + geom_bar(position = "fill")

## "fill" se interpreta en una sola barra vertical cada tipo de automovil por distintas cilindradas

## "Dodge" en cambio muestra una barra para cada tipo de automovil, es decir, una para manual y otra para automático, por cada cilindrada.
...
#En primer lugar, se convierte la variable gear en un factor, gear_f, con la orden factor. Entre paréntesis se indican los niveles que tiene la variable gear. 
mtcars$gear_f <- factor(mtcars$gear, levels = c(3, 4, 5))

#De esta manera se clasifica por cilindrada y, añadiendo gear_f al campo fill, se ve las marchas que tiene cada automovil. 
...
##Si volem fer un canvi, per exemple, volem filtrar la columna carat per 0.25, hem d'utilitzar la ifnestra (bindwith = 0.25). En aquest cas, s'e'exclouen la fila 4 i 5 perquè tenen valors més grans de 0.5
...
#Usando facet_grid(.~cut) la disposición de los histogramas cambian de sentido, es decir, apila las gráficas en vertical. Cambia la distribución.

#Usando facet_grid(cut~.), como se muestra en la gráfica anterior, apila las gráficas de manera horizontal.
```
:::
...
#La mejor opción para comparar distribuciones sería usando facet_grid(cut ~ .) en este sentido, debido a que se puede observar el mismo valor a la vez, ya que enfrenta de manera horizontal cada gráfico y se puede comparar valores rapidamente a diferencia de usar facet_grid(. ~ cut).
...
#Si dentro de la función ggplot, se indica una variable para color, se colorea el contorno de cada boxplot y el interior se muestra blanco.
ggplot(data = diamonds, mapping = aes(y = carat, x = color, colour = color)) +
  geom_boxplot()

#Si se utiliza fill dentro de ggplot, se rellena cada boxplot.
ggplot(data = diamonds, mapping = aes(y = carat, x = color, fill = color)) +
  geom_boxplot()
       
#el que hem de fer si volem pintar tots els boxs és afegir el fill "blue" dintre del geom_boxplot
ggplot(data = diamonds, mapping = aes(y = carat, x = color)) +
...
#Para que las personas dáltonicas puedan diferenciar bien los colores, usaríamos: scale_fill_viridis_d(), ya que presenta mayor diferencia entre las tonalidades.
...
#Añadimos dentro de la capa `labs` (etiquetas), la característica `subtitle` se visualiza como un subtítulo debajo del título del gráfico, `tag` que sirve a modo de cabecera de la imagen, `caption` se muestra como una breve descripción en el pie de la figura.
  
 ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + scale_fill_manual(values = c("sienna1", "orange", "lightseagreen", "orangered", "red4")) + labs(title = "Diamond weight variation", x = "Weight of the diamond", fill = "Quality", subtitle = "Diamond", tag = "Práctica 2 TAB", caption = "Se muestran diferentes distribuciones según el peso y calidad de diamante")
```
:::
...
```{r answer5.3}
#Los que maximizan mejor el data-ink ratio por su fondo claro y las líneas finas son theme_bw() y theme_light(). Hay otras opciones que también serían adecuadas, lo más importante es que el fondo del gráfico no interfiera visualmente con las columnas representadas.
...
#Se carga el dataset economics.
head(economics)
class(economics)

#Se crea una gráfica representando el desempleo respecto al tiempo. Se utiliza geom_line() y se especifica que la linea sea de color rojo como se muestra en el enunciado de la práctica.

ggplot(data = economics, mapping = aes(y=unemploy, x=date)) +
  geom_line(color = "red") + ggtitle("Evolution of unemployment") + scale_y_continuous(name = "Unemployment") + scale_x_continuous(name = "year") 
...
factor_vector <- as.factor(c("1999","1999","2008","2008","1999","1999")) 
str(factor_vector)


ggplot(data = mpg, mapping = aes(y=cty, x = as.factor(year), fill = as.factor(year))) + facet_grid(.~drv) + geom_boxplot() + scale_fill_manual(values = c("#4CC3CD","#FEE883"), name = "Year of manufacture") + ggtitle("Change in fuel economy") + scale_y_continuous(name = "Miles/Gallon")
